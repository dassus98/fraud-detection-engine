...
threshold sweep + sensitivity analysis. Sprint 4.x will layer
stratified metrics on top; the per-call decision metrics
(`economic_cost`, `precision_recall_at_k`, `recall_at_fpr`,
`compute_psi`) continue to live in `src/fraud_engine/utils/metrics.py`
and are imported from there at the consumer site.
"""
