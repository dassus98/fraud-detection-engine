...
|3 — Advanced Features & Models|Complete|12 prompts (3.1.a-b, 3.2.a-b, 3.3.a-d, 3.4.a-c) + audit-and-gap-fill (PR #41) merged; tagged `sprint-3-complete`. Model A (LightGBM, val AUC 0.9376 post-tuning), Model B (FraudNet entity-embedding NN), Model C (FraudGNN GraphSAGE) all trained with MLflow lineage; calibration via isotonic regression; per-model evaluation reports + cross-model comparison.|
|4 — Economic Evaluation|Complete|4 prompts (4.1 EconomicCostModel PR #42, 4.2 StratifiedEvaluator PR #43, 4.3 economic_defaults.yaml + ADR 0003 PR #44, 4.4 run_economic_evaluation script + test-set eval + .env update PR #45) + audit-and-gap-fill PR. Cost-optimal τ = 0.0800 on the 92K test set (matches analytical Bayes limit τ\* ≈ 0.0729 within one grid step); annual savings $28.96M / 1M-txns/month portfolio (58× spec floor); sensitivity spread 0.06 (well under 0.20 ceiling); 100 new tests (35 + 38 + 0 + 27); 4 gap-fixes (.env.example, configs/economic_defaults.yaml, .gitignore allow-list for economic_evaluation.md + 2 figure PNGs, this CLAUDE.md §13 update).|
