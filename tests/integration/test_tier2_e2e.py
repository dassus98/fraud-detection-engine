"""End-to-end Tier-2 feature pipeline test against a 10k stratified sample.

Exercises the full path: real merged frame → cleaner → 10k stratified
sample → Tier-1 + Tier-2 `FeaturePipeline.fit_transform`. Two shape
checks:

- Output validates against `TierTwoFeaturesSchema` (lazy=True).
- Row counts preserved (the Tier-2 generators add columns; they
  never drop rows).

Mirrors `test_tier1_e2e.py`'s `merged_10k_cleaned` fixture pattern —
...
def _build_pipeline() -> FeaturePipeline:
    """Construct the canonical Tier-1 + Tier-2 pipeline."""
...
def test_pipeline_fit_transform_validates_against_schema(
    ...
    """End-to-end: fit on the 10k sample; output validates against TierTwoFeaturesSchema."""
    pipeline = _build_pipeline()
    out = pipeline.fit_transform(merged_10k_cleaned)
    TierTwoFeaturesSchema.validate(out, lazy=True)


def test_pipeline_preserves_row_counts(merged_10k_cleaned: pd.DataFrame) -> None:
    """Tier-1 + Tier-2 generators add columns but never drop rows."""
    pipeline = _build_pipeline()
    out = pipeline.fit_transform(merged_10k_cleaned)
