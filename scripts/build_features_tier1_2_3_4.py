"""Materialise the Tier-1 + Tier-2 + Tier-3 + Tier-4 feature layer.

This script is the canonical entry point that produces
``data/processed/tier4_{train,val,test}.parquet`` plus
``models/pipelines/tier4_pipeline.joblib`` and
...
report can quote a Tier-4 val AUC alongside Sprint 1's 0.9247
baseline, 2.1.d's Tier-1 (0.9165), 2.2.e's Tier-2 (0.9143), and
2.3.c's Tier-3 (0.9063). Spec target for Tier 4: 0.92-0.93.

The Tier-4 build chains all four Tier-1 generators
(`AmountTransformer`, `TimeFeatureGenerator`, `EmailDomainExtractor`,
`MissingIndicatorGenerator`), the three Tier-2 generators
(`VelocityCounter`, `HistoricalStats`, `TargetEncoder`), the three
Tier-3 generators (`BehavioralDeviation`, `ColdStartHandler`,
`NanGroupReducer`), AND the new Tier-4 generator
(`ExponentialDecayVelocity`) into a single fitted `FeaturePipeline`.

Pipeline ordering puts `ExponentialDecayVelocity` at position 10
(between `ColdStartHandler` and `NanGroupReducer`). `NanGroupReducer`
must stay last per its class docstring — it removes V columns; no
downstream generator may reference them. EWM column names start with
the entity name (e.g. `card1_v_ewm_lambda_0.05`), not `V`, so
`NanGroupReducer`'s `_detect_v_columns` regex never matches them and
the EWM columns survive the reduction step intact.
...
Trade-offs considered:
    - **Fit on train only.** `pipeline.fit_transform(train)` runs
      OOF target encoding + EWM read-before-push on training rows
      AND fits the full-train state; later `transform(val)` /
      `transform(test)` apply the frozen state in read-only mode.
      Same train-only discipline as Tier-1/2/3.
    - **Schema validation against `TierFourFeaturesSchema`.** Each
      split's output is validated *before* parquet write. Missing
      V columns (dropped by `NanGroupReducer`) pass through via
      `strict=False` inherited from `MergedSchema`.
    - **Quick LightGBM retrain.** Spec target val AUC 0.92-0.93.
      Object / string columns (provider, tld) are dropped; the
      `*_target_enc` and `*_v_ewm_*` columns are float and survive.

Usage:
    uv run python scripts/build_features_tier1_2_3_4.py
"""
...
_PIPELINE_FILENAME: Final[str] = "tier4_pipeline.joblib"
_PIPELINE_SUBDIR: Final[str] = "pipelines"
_PROCESSED_PREFIX: Final[str] = "tier4"

# Columns dropped from the LightGBM retrain feature set. Identical
# to the Tier-1/2/3 builds — Tier-4 introduces no new non-feature
...
def _build_pipeline() -> FeaturePipeline:
    """Construct the canonical Tier-1 + Tier-2 + Tier-3 + Tier-4 `FeaturePipeline`.
    ...
    Order matters:
        ...
        11. NanGroupReducer              — drop redundant V columns (LAST; see class docstring)
    """
...
def _validate_split(name: str, df: pd.DataFrame) -> None:
    """Validate one split against `TierFourFeaturesSchema`."""
    TierFourFeaturesSchema.validate(df, lazy=True)
    get_logger(__name__).info(
        "build_features_tier4.validated",
        split=name,
        rows=int(df.shape[0]),
        cols=int(df.shape[1]),
    )
...
def _write_processed_parquets(
    ...
    """Write each split to ``processed_dir/tier4_{name}.parquet``."""
...
@click.command()
def main() -> None:
    """Build the Tier-1 + Tier-2 + Tier-3 + Tier-4 feature layer; report a quick val AUC."""
    settings = get_settings()
    settings.ensure_directories()
    logger = get_logger(__name__)

    with Run("build_features_tier4", settings=settings) as run:
        ...
        # 3. Validate each split against the Tier-4 schema.
        ...
        # 5. Save pipeline + manifest with the tier-4 filename.
        ...
        logger.info(
            "build_features_tier4.persisted",
        ...
        run.log_metric("tier4_val_auc", val_auc)
        run.log_metric("tier4_n_train", float(len(train_out)))
        run.log_metric("tier4_n_val", float(len(val_out)))
        run.log_metric("tier4_n_test", float(len(test_out)))
        run.log_metric("tier4_n_features_total", float(train_out.shape[1]))

        # 7. Echo a summary table.
        click.echo(click.style("build_features_tier4: GREEN", fg="green", bold=True))
