"""Sprint 3 FraudNet (Model B) training pipeline.

End-to-end: load `data/processed/tier5_*.parquet` -> build transductive
entity vocabularies (card1, addr1, DeviceInfo) over train+val+test ->
fit a `FraudNetModel` on train with focal loss + early stopping on
val AUC -> evaluate on val + test (ROC-AUC / PR-AUC / log loss /
Brier / ECE) -> time single-row inference latency -> save the model
to `models/sprint3/fraudnet/` -> emit
`reports/model_b_training_report.md` and a per-epoch learning-curve
PNG.

Business rationale:
    Model B is the diversity model in the Sprint-3 stack. The full
    training run produces the artifact Sprint 4's evaluator and
    Sprint 5's shadow harness load. Centralising the orchestration
    here means the integration test exercises the same code path
    as production via a `--quick` smoke (50 K stratified subsample,
    10 epochs, smaller patience) — no separate code branches.

Trade-offs considered:
    - **No calibration in this script.** 3.3.c's calibration toolkit
      is reusable but applied here would muddy the diversity
      comparison (Model A o+ Model B ensemble blending is most
      legible on raw probabilities). Sprint 4 owns calibration of
      Model B if the ensemble math demands it.
    - **No Optuna sweep.** Single-config train. The 3.3.b Optuna
      harness is LightGBM-specific; an analogous PyTorch sweep is
      out of scope here. The defaults already encode reasonable
      choices (focal alpha=0.25, gamma=2.0, embed_dim=32, lr=1e-3).
    - **Single MLflow run** (`model_b_train`). Per-epoch metrics
      logged alongside the run; no nested children.
    - **Latency reporting, not gating.** FraudNet is shadow-
      deployable per CLAUDE.md §3 — it isn't on the 15 ms hot path
      that Model A bears. We record p50/p95/p99 for the record but
      don't gate the run on a budget.
    - **`--quick` is the same code path as production.** Only
      `sample_size`, `max_epochs`, and `early_stopping_patience` are
      overridden via the CLI. The integration test uses the
      Pythonic `train_pipeline` entry point with the same
      overrides.

Usage:
    uv run python scripts/train_neural.py            # full run (~10-20 min)
    uv run python scripts/train_neural.py --quick    # 50 K smoke (~30-60 s)
"""
...
from fraud_engine.models.neural_model import (  # noqa: E402
    ENTITY_COLUMNS,
    FraudNetModel,
)
...
# Output paths.
_MODELS_SUBDIR: Final[str] = "sprint3/fraudnet"
_TRAINING_REPORT_PATH: Final[Path] = (
    Path(__file__).resolve().parents[1] / "reports" / "model_b_training_report.md"
)
_TRAINING_CURVES_PATH: Final[Path] = (
    Path(__file__).resolve().parents[1] / "reports" / "figures" / "model_b_training_curves.png"
)

# Defaults (mirror neural_model module constants).
_DEFAULT_EPOCHS: Final[int] = 50
...
# `--quick` overrides.
_SMOKE_SAMPLE_SIZE: Final[int] = 50_000
_SMOKE_EPOCHS: Final[int] = 10
_SMOKE_PATIENCE: Final[int] = 3

# Acceptance gates referenced in the auto-generated report.
_VAL_AUC_FLOOR: Final[float] = 0.5  # catastrophic floor for diversity model
_CONVERGENCE_TOLERANCE: Final[float] = 0.01  # final-epoch within 1% of best

# MLflow run naming.
_MLFLOW_RUN_NAME: Final[str] = "model_b_train"

# Non-feature columns mirror the LightGBM convention.
...
def _select_columns_for_fraudnet(df: pd.DataFrame) -> list[str]:
    """Return the columns FraudNetModel ingests: entity IDs + numerics.

    Mirrors `train_lightgbm._select_features` for the numeric subset
    AND keeps the three categorical entity columns (which FraudNet
    routes through embeddings instead of dropping).
    """
    cols: list[str] = []
...
def _measure_inference_latency(
    model: FraudNetModel,
    sample_X: pd.DataFrame,  # noqa: N803 — sklearn convention
...
def _save_training_curves(
    ...
    ax1.set_title(f"FraudNet training curves (n_epochs = {len(epochs)})")
    fig.tight_layout()
...
def _render_training_report(  # noqa: PLR0915 — linear markdown builder
    *,
    ...
    lines: list[str] = []
    lines.append("# Model B - FraudNet training report")
    lines.append("")
    lines.append("- **Generated by:** `scripts/train_neural.py`")
    ...
    lines.append(f"- **card1 vocab:** {result.card1_vocab_size:,}")
    lines.append(f"- **addr1 vocab:** {result.addr1_vocab_size:,}")
    lines.append(f"- **DeviceInfo vocab:** {result.deviceinfo_vocab_size:,}")
    ...
    lines.append("")
    lines.append(
        "Single-row `predict_proba`, n = "
        f"{_LATENCY_N_SAMPLES:,} random val rows. FraudNet is shadow-"
        "deployable (CLAUDE.md §3); no production-path latency budget."
    )
    lines.append("")
    ...
    lines.append(
        ...
        f"{max(result.val_auc_history):.{_AUC_DIGITS}f})"
    )
    ...
    lines.append("")
    lines.append("- Calibration of FraudNet outputs (3.3.c toolkit reusable).")
    lines.append("- Ensemble with Model A; cost-curve evaluation; threshold optimisation.")
    lines.append("- SHAP / interpretability (Sprint 5 deliverable for Model A).")
    lines.append("")
...
def train_pipeline(  # noqa: PLR0913, PLR0915 — single-file orchestration; the knobs match the CLI
    *,
    ...
    Args:
        ...
        epochs: Cap on training epochs. Default 50.
        batch_size: Mini-batch size. Default 2048.
        lr: Adam learning rate. Default 1e-3.
        embed_dim: Embedding dimension per entity. Default 32 (per spec).
        ...
        sample_size: If not None, stratified-subsample train/val/test
            to ~this many rows (val/test get sample_size//5). Used by
            the `--quick` smoke + integration test.
        random_state: Seed for torch + numpy + the subsampling.
            Defaults to `settings.seed`.
    ...
    feature_cols = _select_columns_for_fraudnet(train)
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=_MLFLOW_RUN_NAME):
        mlflow.set_tag("stage", "sprint3_train_modelb")
        mlflow.set_tag("model_class", "FraudNetModel")
        ...
        # Pass val + test entity columns into vocab construction
        # (transductive choice; documented in neural_model.py).
        model.fit(train_x, train_y, val_x, val_y, extra_frames=(val_x, test_x))
        ...
        # --- Latency ---
        p50, p95, p99, latencies = _measure_inference_latency(
            model, val_x, n_samples=n_latency_samples, seed=seed
        )
        mlflow.log_metric("inference_p50_ms", p50)
        mlflow.log_metric("inference_p95_ms", p95)
        mlflow.log_metric("inference_p99_ms", p99)
        # latency histogram is implicit in p50/p95/p99 + the curves
        # PNG lives separately as the per-epoch artefact.
        ...
        result = TrainingResult(
            ...
            card1_vocab_size=len(model.card1_vocab_ or {}),
            addr1_vocab_size=len(model.addr1_vocab_ or {}),
            deviceinfo_vocab_size=len(model.deviceinfo_vocab_ or {}),
        ...
        _ = latencies  # explicit no-op to match shape with train_lightgbm.py
...
@click.option("--embed-dim", type=int, default=_DEFAULT_EMBED_DIM, show_default=True)
@click.option(
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Run a smoke: 50 K stratified subsample + 10 epochs.",
)
def main(  # noqa: PLR0913 — Click decorator-driven; each kwarg is a CLI flag the user passes
    epochs: int,
    batch_size: int,
    lr: float,
    embed_dim: int,
    ...
    """Run the full FraudNet training pipeline."""
    ...
    click.echo(
        ...
        f"{result.latency_p50_ms:.2f}/{result.latency_p95_ms:.2f}/"
